...
function bpCurl($url, $apiKey, $post = false) {
	...
	curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, 1); //false for testing locally, 1 for production verify certificate
	curl_setopt($curl, CURLOPT_SSL_VERIFYHOST, 2); //false for testing locally, 2 for production check existence of CN and verify that it matches hostname
